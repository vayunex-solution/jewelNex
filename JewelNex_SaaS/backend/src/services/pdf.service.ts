...
function formatCurrency(amount: number | string | null | undefined, symbol = '₹'): string {
...
export class PDFService {
  static async generateInvoicePDF(invoiceId: string): Promise<Readable> {
    ...
    cols.forEach(col => {
      doc.fillColor(WHITE).font('Helvetica-Bold').fontSize(7)
        .text(col.label, col.x, y + 7, { width: col.width, align: col.align as any });
    });
    y += 22;
    ...
    items.forEach((item, idx) => {
      ...
      rowData.forEach(cell => {
        doc.fillColor(DARK).font('Helvetica').fontSize(7)
          .text(cell.val, cell.x, rowY + 5, { width: cell.w, align: cell.align as any });
      });
    ...
    if (invoice.payments && invoice.payments.length > 0) {
      ...
      if (Math.abs(balance) > 0.01) {
        doc.fillColor(balance > 0 ? '#c0392b' : '#27ae60').font('Helvetica-Bold').fontSize(8)
          .text(balance > 0 ? `Balance Due: ${formatCurrency(balance)}` : `Advance: ${formatCurrency(Math.abs(balance))}`,
            margin, y);
        y += 14;
      }
    }

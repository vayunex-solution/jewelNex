...
class InvoiceService {
     ...
     */
    static async reverseInvoice(invoiceId, userId) {
        try {
            return await database_1.default.$transaction(async (tx) => {
                ...
                await accounting_service_1.AccountingService.generateReversalVoucher(invoiceId, tx);
     ...
     */
    static async editDraft(id, dto, userId) {
        const existing = await database_1.default.invoice.findUnique({
            where: { id },
            select: { status: true }
        });
        if (!existing) {
     ...
     */
    static async postDraft(id, userId, payments = []) {
        try {
        ...
        `;
                // 2. Generate double-entry vouchers
                await accounting_service_1.AccountingService.generateInvoiceVoucher(id, tx);
                // 3. Fetch the created invoice to return
                return await tx.invoice.findUnique({
                    where: { id },
                    include: { items: true, payments: true, customer: true }

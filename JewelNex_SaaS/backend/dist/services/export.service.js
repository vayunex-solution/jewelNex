...
function applyHeaderStyle(row, bgColor = HEADER_BG) {
    row.eachCell(cell => {
...
function addWorkbookMeta(workbook, title, company) {
...
class ExportService {
    // ─── TRIAL BALANCE XLSX ────────────────────────────────────────────
    static async exportTrialBalanceXLSX(date) {
        const tb = await accounting_service_1.AccountingService.getTrialBalance(date);
        ...
        tb.rows.forEach((row, idx) => {
            ...
            if (row.debit > 0)
                r.getCell('debit').font = { size: 9, bold: true };
            if (row.credit > 0)
                r.getCell('credit').font = { size: 9, bold: true };
        });
        // Totals row
    ...
    // ─── DAY BOOK XLSX ─────────────────────────────────────────────────
    static async exportDayBookXLSX(startDate, endDate) {
        const dayBook = await accounting_service_1.AccountingService.getDayBook(startDate, endDate);
        const workbook = new exceljs_1.default.Workbook();
    ...
    // ─── GENERAL LEDGER XLSX ───────────────────────────────────────────
    static async exportGeneralLedgerXLSX(accountId, startDate, endDate) {
        const ledger = await accounting_service_1.AccountingService.getGeneralLedger(accountId, startDate, endDate);
        const workbook = new exceljs_1.default.Workbook();
    ...
    static async exportInvoiceRegisterCSV(startDate, endDate) {
        const whereClause = { status: { not: 'DRAFT' } };

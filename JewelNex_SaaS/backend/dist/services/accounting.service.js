...
class AccountingService {
    ...
    static async initializeChartOfAccounts() {
        ...
        return await database_1.default.$transaction(async (tx) => {
            ...
            for (const head of headsSeed) {
                ...
                if (!groupId)
                    throw new Error(`Group not found: ${head.groupName}`);
     ...
     */
    static async createVoucher(dto, txContext) {
        const client = txContext || database_1.default;
        let totalDebit = 0;
        let totalCredit = 0;
     ...
     */
    static async getOrCreateCustomerHead(customerId, customerName, tx) {
     ...
     */
    static async generateInvoiceVoucher(invoiceId, tx) {
        const invoice = await tx.invoice.findUnique({
            where: { id: invoiceId },
            include: { customer: true, payments: true }
        });
        if (!invoice)
            throw new Error('Invoice not found for voucher posting.');
    ...
    static async generateReversalVoucher(invoiceId, tx) {
        ...
        for (const voucher of originalVouchers) {
            const reversedEntries = voucher.entries.map((entry) => ({
                accountId: entry.accountId,
     ...
     */
    static async getAccountBalance(accountId, txContext) {
        const client = txContext || database_1.default;
     ...
     */
    static async getCustomerBalance(customerId, txContext) {
        const client = txContext || database_1.default;
        const head = await client.accountHead.findUnique({
            where: { customerId }
        });
     ...
     */
    static async getDayBook(startDate, endDate, txContext) {
        const client = txContext || database_1.default;
        const whereClause = {};
     ...
     */
    static async getGeneralLedger(accountId, startDate, endDate, txContext) {
        const client = txContext || database_1.default;
        const head = await client.accountHead.findUnique({
            where: { id: accountId },
            include: { group: true }
        ...
        if (startDate || endDate) {
            whereClause.voucher = {};
            if (startDate)
                whereClause.voucher.date = { gte: new Date(startDate) };
            if (endDate) {
     ...
     */
    static async getTrialBalance(date, txContext) {
        const client = txContext || database_1.default;
     ...
     */
    static async getProductWeightedAverageCost(productId, txContext) {
        const client = txContext || database_1.default;
     ...
     */
    static async getProfitLoss(startDate, endDate, txContext) {
        const client = txContext || database_1.default;
        const rangeClause = {};
        if (startDate)
            rangeClause.gte = new Date(startDate);
        if (endDate)
            rangeClause.lte = new Date(endDate);
     ...
     */
    static async getBalanceSheet(date, txContext) {
        const client = txContext || database_1.default;
        const dateLimit = date ? new Date(date) : undefined;
        const heads = await client.accountHead.findMany({
            include: { group: true }
